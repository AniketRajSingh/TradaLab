...
class EmailMessageAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if use_html_template and html_template:
            ...
            email = EmailMultiAlternatives(subject, text_content, 'no-reply@tradalab.tech', recipient_emails)
            email.attach_alternative(rendered_html, "text/html")
            email.send()
        else:
            # Send plain text email
            email = EmailMultiAlternatives(subject, message, 'no-reply@tradalab.tech', recipient_emails)
            email.send()
